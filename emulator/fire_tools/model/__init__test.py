...
def interpolator(w, slices=4, interpolation='quadratic'):
    """
    Takes in an array of weather features, w, with shape (timesteps, features).
    It returns an array of length (timesteps * slices, features), with points equidistantly spaced.
    """
    w = tf.keras.layers.Input(shape=[TIME_STEPS, 4], name='weather_interpolation')
    
    # need to 1D interpolate
    #print(w.shape)
    bs, length, num_feat = w.shape
    
    if length is None: # 
        length = 2
    
    x = np.arange(0, (length-1)*slices)
    x = (x+0.5) / slices

    x0 = np.arange(0, length)
    
    w_interpolated = tf.zeros(((length-1)*slices, num_feat))
    
    for feat in range(num_feat):
        
        f = interp1d(x0, w[:, feat], kind=interpolation) # create an interpolator object
        
        y = f(x) # interpolated weaether values

        w_interpolated[:, feat] = y
...
class FCN_RNN7(tf.keras.layers.Layer):
    ...
    def call(self, inputs, state):
        ...
        w_expanded = tf.math.multiply(weather_shape, w_inputs)
        ...
        slices = 1#4 # mvoe sub-intervals to outside of loop (interpolate weather)
        ...
        for j in range(slices): # j is a slice of time
            ...
            arrival_edges = tf.where(arrival <= 0.01, -1.0, arrival)
            ...
            # ERROR redundant concat of arrival with features, arrival can be removed from concat above
            # CORRECTION. not entirely redundant since conv_fs is a 1D layer. Performance seems greatly improved 
            # retaining arrival in the above feature stack
...
def cnn_rnn_model(activation='relu'):
    ...
    ##w = weather[:, 0::2] # initial weather at each interval
    ##w = w.append(weather[-1, 1::2]) # append final weather at final interval 
    
    # interpolate weather
    
    w = weather[..., 0::2]
    w_end = tf.expand_dims(weather[..., -1, 1::2],0)
    ##w = np.concatenate((w, w_end), axis=0)
    w = tf.concat([w, w_end], axis=0)
    
    slices = 4 # ratio of points to interpolate
    w = interpolator(w, slices)
    
    # weather feature engineering
        
    w = tf.keras.layers.Conv1D(kernel_size=1, strides=1, filters=6, activation=activation, padding='same')(w)
    w = tf.keras.layers.Conv1D(kernel_size=1, strides=1, filters=6, activation=activation, padding='same')(w)
