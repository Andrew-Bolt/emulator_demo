...
def get_landclass_balance(lc_path, at_path, last_band):
    
    """
    input: the processed lc_path and at_path
    return: percentage of each landclass in each sample
            percentage of burnt landclass in each sample
    """
    
    with rasterio.open(at_path) as src:     
        
        #arrival_image = src.read(band)
        arrival_image = src.read(last_band)

        max_val = np.max(arrival_image)

        arrival_image = np.where(arrival_image < max_val-1, arrival_image, 0)
        arrival_image = np.where(arrival_image > 0.01, 1, 0)
    
    with rasterio.open(lc_path) as src:

        land_image = src.read()

        landclass = np.swapaxes(land_image, 0, -1)    # put channels last
        landclass = np.swapaxes(landclass, 0, 1) # transpose to match arrival
    
    # pixel count for each landclass
    class_count = np.sum(landclass, axis=(0,1))
    
    # burnt pixel count for each landclass
    
    product = landclass * np.expand_dims(arrival_image, -1)
    burnt_class_count = np.sum(product, axis=(0,1))
    
    
    return class_count, burnt_class_count
    
    
    
    
def transform_crs(src_image, tgt_image, proc_path, resampling=Resampling.nearest, num_threads=4, no_data=65526, data_type=None):
    """
...
def transform_time_series(data_csv, proc_path, interval, pixel_length):
    ...
    # convert wind speed and direction to vector components
        ...
        # converting to west-east, south-north, travel vectors
...
def transform_arrivaltime_map(at_raw, proc_path, interval, num_intervals):    
    ...
    with rasterio.Env():
        ...
        profile.update(
            nodata=0.0,
            dtype='float32',
            count=num_intervals+1) # number of bands
        
        ## should i just increase count by 1 to accomodate the new init/final config?

        with rasterio.open(proc_path + os.path.basename(at_raw), 'w', **profile) as dst:

            area_final = []
            area_start = []
            for band in range(num_intervals+1):
                ...
                if band == 0:
                    ...
                    area = sum(sum(img_clip > 0))
                    if band < num_intervals:
                        area_start.append(area) # append to all but last trial - final position
...
def preprocess_data(raw_dir, proc_dir, heightmap_dir, landclass_dir, weather_dir, data_pkl):
    ...
    if not(os.path.exists(cwd + '/' + data_pkl)):
        ...
        columns = ['sample_name', 
                   ...
                   'weather_format',
                   'class_count', # the pixel counts of each class in a trial
                   'burnt_class_count'] # the number of pixels of a given class that were burnt
    ...
    for (dirpath, dirnames, filenames) in os.walk(raw_dir):
        ...
        # get a list oo._io.DatasetWriterBase.write()f just the arrival images
        ...
        for ind, trial in zip(dataframe.index, dataframe.sample_name):
            ...
            if hm_raw != None:
                ...
                if os.path.basename(hm_raw) != os.path.basename(str(hm_proc)):
                    ...
                    transform_crs(hm_raw, at_raw, proc_path, resampling='bilinear', data_type=rasterio.float32)

                    # rescale_heights
                    hm_proc = proc_path + os.path.basename(hm_raw)
                    max_height, min_height = rescale_heights(hm_proc)
...
def create_landclass_balance(master_dataframe):
    
    dataframe = pd.read_pickle(master_dataframe)
    
    for trial_number in dataframe.index:

        lc_path_proc = dataframe.loc[trial_number, 'landclass_proc']
        at_path_proc = dataframe.loc[trial_number, 'arrival_time_proc']
        last_band = dataframe.loc[trial_number, 'num_intervals'] + 1
        
        class_count, burnt_class_count = get_landclass_balance(lc_path_proc, at_path_proc, last_band)
        
        # update dataframe
        
        dataframe.loc['trial_number', 'class_count'] = [[class_count]]
        dataframe.loc['trial_number', 'burnt_class_count'] = [[burnt_class_count]]
        
        dataframe.to_pickle(master_dataframe)
...
def create_trial_sample_points(trials_pkl, N=1000):
    ...
    for trial_num in trial_data.index: # loop over all samples
        ...
        with rasterio.open(at_path) as src:
            ...
            img = src.read(int(interval) + 2) #+2 when using new init/final arrival config
