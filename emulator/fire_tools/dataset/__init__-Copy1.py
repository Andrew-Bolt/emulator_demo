...
'g_min_RH', 'g_min_RH', 'g_min_DF', 'g_min_CG']]
    ...
    weather = np.stack((wind_x_init, wind_x_final, wind_y_init, wind_y_final, temp_init, temp_final, RH_init, RH_final, DF, CG))
    ...
    features = (arrival_init, weather, heightmap, landclass, transform)
...
def load_topo(trial_number, dataframe, window_size=128, crop=False):
    ...
    heightmap = tf.squeeze(heightmap, 0)
    heightmap   = tf.cast(heightmap, tf.float32) # convert to float type
    
    #print(np.shape(heightmap))
    #print(type(heightmap))
    #print('above is heightmap shape')
    
    #print(np.shape(landclass))
    #print(type(landclass))
    #print('above is landclass shape')
...
def load_predict(trial_number, dataframe, transform=[0.0, 0.0, 0.0, 0.0]):
    ...
    arrival_init = load_image(at_path_init, None) # all bands
    arrival_init = tf.transpose(arrival_init,perm=[1,2,0])
...
def transform_trial(features, target):
    ...
    arrival, weather, heightmap, landclass, transform = features
    ...
    # stack weather
    #temporarily disable changing the weather
    weather = tf.concat((wind_x_init, wind_x_final, wind_y_init, wind_y_final, weather_rest), 0)
    
    # return transformed images
    features = (arrival, weather, heightmap, landclass)
